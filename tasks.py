...
def botprogram():
    ...
    schedule.every().thursday.at("14:36").do(postit,Resource) #Schedule library function to run postit function at friday at 10am (8am GMT). The Resource array is passed as a parameter to the postit function.
    ...
    while True: #'Runtime infinite loop'
        ...
        if index == -1: #if the telegram user does not have an associated resource object in the Resource array, their index is defaulted to -1
            try:
                ...
                try:
                    cur.execute("SELECT name FROM salesforce.resource__c WHERE name = '%s'" % (name))
                    query_result = cur.fetchone()
